...
- 桌面快捷方式（普通权限、无黑框；这样才能把桌面图标拖进窗口）、托盘、开机自启、单实例、F5 刷新、规则「运行中」状态
- 「设置」页可一键把快捷方式放到桌面
...
- **不要以管理员运行主窗口。** Windows 禁止从资源管理器把文件拖进管理员窗口（会出现红白禁放光标）。需要绑定时再在窗口右上角点「管理员」
...
桌面双击 **网口分流** 快捷方式（普通权限、无控制台），或双击 `启动网口分流.bat`，或：
...
1. 用普通方式打开本程序（不要「以管理员运行」，否则拖不进桌面图标）
2. 确认顶部同时出现「有线网」和「无线网」
3. 把桌面快捷方式拖到对应网络卡片；或添加规则：
...
4. **不要对带反作弊的游戏使用「网卡绑定」。** 无畏契约、星穹铁道等会拦截注入，分流不会生效，还可能被当成外挂。请把游戏留在系统默认网卡，用本工具分流抖音、浏览器、WorkBuddy、VPN 客户端。
...
13. **主窗口不要用管理员。** 管理员窗口无法接收桌面拖放（红白禁放光标）。浏览器/抖音走代理模式不需要管理员；WorkBuddy 等网卡绑定失败时，再点右上角「管理员」。
...
- 双击 bat 一闪而过：找不到 Python。bat 会优先用 `%LocalAppData%\Programs\Python\Python3xx\pythonw.exe`。
- 桌面图标拖到窗口上出现红白禁放光标：窗口正在以管理员运行。关掉后用桌面「网口分流」普通打开（不要勾选以管理员运行）。
